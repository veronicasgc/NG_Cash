...
describe("Account Business - Login",()=>{
    beforeEach(()=>{
        jest.spyOn(Authenticator.prototype, "generateToken").mockReturnValue("token-fake")
    })
    afterEach(()=>{
...
describe("AccountBusiness - AccountById",()=>{
    beforeEach(()=>{
        ...
        jest.spyOn(Authenticator.prototype, "getTokenData").mockReturnValue({id:123456})
        jest.spyOn(AccountDatabase.prototype, "selectAccountById").mockResolvedValue(
            {
                id: 2,
                balance:1000
            }
    ...
    test("Should return error when token does not exist", async()=>{
       ...
       test("Should return error when account does not exist", async()=>{
        jest.spyOn(AccountDatabase.prototype, "selectAccountById").mockResolvedValue(undefined)
        const accountBusiness = new AccountBusiness()
    ...
    test("Should return error when token is invalid", async()=>{
        jest.spyOn(Authenticator.prototype, "getTokenData").mockImplementation((token)=>{
            if(token === "token-fake"){
                return {id:123456}
            }
